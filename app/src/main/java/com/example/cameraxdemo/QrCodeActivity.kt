...
class QrCodeActivity : AppCompatActivity() {
    ...
    private fun startCamera() {
        ...
        cameraController.setImageAnalysisAnalyzer(cameraExecutor,
                MlKitAnalyzer(listOf(barcodeScanner), COORDINATE_SYSTEM_VIEW_REFERENCED,ContextCompat.getMainExecutor(this@QrCodeActivity)) { result: MlKitAnalyzer.Result? ->
...
//                previewView.overlay.add(qrCodeDrawable)
    ...
    override fun onActivityResult(requestCode: Int, resultcode: Int, @Nullable data: Intent?) {
        ...
        if (requestCode == PICK_IMAGE_REQUEST&&resultcode == RESULT_OK &&data!=null){
            ...
            if (uri!=null){
                //识别图片二维码
                val inputImage = InputImage.fromFilePath(this@QrCodeActivity,uri)
                barcodeScanner.process(inputImage).addOnSuccessListener { barcodeResults ->
                        //没有数据返回,继续扫描
                        val previewView: PreviewView = bind.viewFinder
                        if ((barcodeResults == null) || (barcodeResults.size == 0) || (barcodeResults.first() == null)) {
                            previewView.overlay.clear()
